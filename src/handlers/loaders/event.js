...
module.exports = (client) => {
    ...
    if (client.shard.ids[0] === 0) console.log(kleur.blue(kleur.bold(`System`)), (kleur.white(`>>`)), (kleur.green(`Loading events`)), (kleur.white(`...`)))
    if (client.shard.ids[0] === 0) console.log(`\u001b[0m`);

    fs.readdirSync('./src/events').forEach(dirs => {
        const events = fs.readdirSync(`./src/events/${dirs}`).filter(files => files.endsWith('.js'));

        if (client.shard.ids[0] === 0) console.log(kleur.blue(kleur.bold(`System`)), (kleur.white(`>>`)), kleur.red(`${events.length}`), (kleur.green(`events of`)), kleur.red(`${dirs}`), (kleur.green(`loaded`)));
