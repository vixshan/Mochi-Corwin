...
module.exports = async client => {
  ...
  client.transcript = async function (interaction, channel) {
    ...
    const dom = new jsdom.JSDOM(
      template.replace('{{TITLE}}', interaction.channel.name)
    )
    ...
    document.getElementsByClassName('preamble__guild-icon')[0].src =
    ...
    for (const message of Array.from(messages.values()).sort(
      ...
      if (message.reference?.messageId) {
        ...
        reference.innerHTML = `<img class="chatlog__reference-avatar" src="${
          author.avatarURL() ?? static.defaultPFP
                ...
                <span class="chatlog__reference-name" title="${author.username.replace(
                  ...
                  author.bot
                    ? `<span class="chatlog__bot-tag">BOT</span> ${xss(
                        author.username
                      )}`
                    : xss(author.username)
                }</span>
    ...
    const fileTrans = new discord.AttachmentBuilder(
      Buffer.from(dom.serialize()),
      { name: `${interaction.channel.id}.html` }
    )
