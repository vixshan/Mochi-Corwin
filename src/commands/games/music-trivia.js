...
module.exports = async (client, interaction, args) => {
  ...
  if (!number || isNaN(number))
    return client.errUsage(
      { usage: 'music-trivia [amount of numbers]', type: 'editreply' },
      interaction
    )
  ...
  if (!channel)
    return client.errNormal(
      { error: `You're not in a voice channel!`, type: 'editreply' },
      interaction
    )
  ...
  if (
    ...
    return client.errNormal(
      { error: `You are not in the same voice channel!`, type: 'editreply' },
      interaction
    )

  if (interaction.client.playerManager.get(interaction.guild.id))
    return client.errNormal(
      {
        error: `You can't use this while a track is playing!`,
        type: 'editreply',
      },
      interaction
    )
  ...
  if (interaction.client.triviaManager.get(interaction.guildId))
    return client.errNormal(
      { error: `There is already a trivia in play!`, type: 'editreply' },
      interaction
    )
